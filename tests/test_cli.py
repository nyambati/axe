...
def test_tree_command_file_not_found():
    ...
    with patch('builtins.open', mock_open()) as mock_file:
        ...
        with patch('axe.tree.print') as mock_print:
            exit_code = tree(mock_args)
            assert exit_code == 1
            mock_print.assert_called_once_with('[red]Error: Config file not found: nonexistent.yaml[/red]')
...
def test_tree_command_yaml_error():
    ...
    with patch('builtins.open', mock_open(read_data="invalid:yaml")) as mock_file:
        with patch('axe.tree.print') as mock_print:
            exit_code = tree(mock_args)
            assert exit_code == 1
            mock_print.assert_called_once_with('[red]No route configuration found in the YAML file[/red]')
...
def test_route_tree_structure():
    ...
    assert any("email" in str(child.label) for child in root_node.children)  # Nested route
    
    # Verify the continue flag is properly displayed
    continue_node = next((c for c in root_node.children if "Continue" in str(c.label)), None)
...
def test_route_tree_structure_with_matchers():
    ...
    root_node = tree.children[0]
    matchers_node = next((c for c in root_node.children if "Matchers" in str(c.label)), None)
